...
export class ChildProcessTestRunnerProxy {
    ...
    constructor(options, fileDescriptions, sandboxWorkingDirectory, loggingServerAddress, pluginModulePaths, getLogger, idGenerator) {
        ...
        this.worker = ChildProcessProxy.create(new URL('./child-process-test-runner-worker.js', import.meta.url).toString(), loggingServerAddress, options, fileDescriptions, pluginModulePaths, sandboxWorkingDirectory, ChildProcessTestRunnerWorker, options.testRunnerNodeArgs, getLogger, idGenerator);
    }
    capabilities() {
