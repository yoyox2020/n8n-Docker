...
import { ERROR_CODES, isErrnoException, notEmpty, } from '@stryker-mutator/util';
    ...
    resolveTestFiles(inputFileNames) {
        ...
        for (const pattern of this.testFilePatterns) {
            ...
            if (matchedFiles.length === 0) {
                this.log.warn(`Glob pattern "${pattern}" did not match any test files.`);
            }
        ...
        for (const pattern of this.mutatePatterns) {
            if (pattern.startsWith(IGNORE_PATTERN_CHARACTER)) {
                ...
                for (const [fileName, file] of files) {
                    mutateInputFileMap.set(fileName, this.unionFileDescriptions(file, mutateInputFileMap.get(fileName)));
                }
            }
        }
        if (targetMutatePatterns) {
            ...
            for (const pattern of targetMutatePatterns) {
                ...
                for (const [fileName, description] of files) {
                    const intersected = this.intersectFileDescriptions(mutateInputFileMap.get(fileName), description);
    ...
    intersectFileDescriptions(first, second) {
        if (Array.isArray(first.mutate) && Array.isArray(second.mutate)) {
            ...
            const intersectedRanges = first.mutate
                .flatMap((firstRange) => secondMutate.map((secondRange) => {
                ...
                const startColumn = firstRange.start.line === startLine
                    ? firstRange.start.column
                    : secondRange.start.column;
                const endColumn = firstRange.end.line === endLine
