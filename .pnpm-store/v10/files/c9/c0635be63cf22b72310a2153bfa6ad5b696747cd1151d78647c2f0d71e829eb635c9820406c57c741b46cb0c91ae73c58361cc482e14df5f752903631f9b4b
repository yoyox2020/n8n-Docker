...
exports.OracleVS = exports.dropTablePurge = exports.createIndex = exports.createTable = exports.DistanceStrategy = exports.VectorElementFormat = exports.VectorType = exports.generateWhereClause = void 0;
const node_crypto_1 = __importDefault(require("node:crypto"));
const oracledb_1 = __importDefault(require("oracledb"));
const vectorstores_1 = require("@langchain/core/vectorstores");
const documents_1 = require("@langchain/core/documents");
const math_1 = require("@langchain/core/utils/math");
function generateWhereClause(dbFilter, bindValues) {
    // Handle $and
...
// eslint-disable-next-line no-instanceof/no-instanceof
value instanceof oracledb_1.default.SparseVector;
function normalizeVectorTypeValue(value) {
    const normalized = value?.toUpperCase();
    if (normalized === exports.VectorType.DENSE || normalized === exports.VectorType.SPARSE) {
        return normalized;
    }
    return exports.VectorType.DENSE;
}
function normalizeVectorFormat(value, vectorType) {
    if (!value) {
        return exports.VectorElementFormat.FLOAT32;
    }
    const normalizedValue = value === exports.VectorElementFormat.FLEX
        ? exports.VectorElementFormat.FLEX
        : value.toUpperCase();
    const validFormats = Object.values(exports.VectorElementFormat);
    const format = validFormats.includes(normalizedValue)
        ? normalizedValue
        : exports.VectorElementFormat.FLOAT32;
    if (vectorType === exports.VectorType.SPARSE && format === exports.VectorElementFormat.BINARY) {
...
function buildVectorColumnDefinition(embeddingDim, customization) {
    ...
    if (vectorType === exports.VectorType.SPARSE && customization?.format === undefined) {
    ...
    if (format === exports.VectorElementFormat.BINARY && embeddingDim % 8 !== 0) {
        throw new Error("BINARY vector format requires dimensions to be a multiple of 8.");
    }
    const dimensionSegment = String(embeddingDim);
    const formatSegment = format ?? exports.VectorElementFormat.FLOAT32;
    if (vectorType === exports.VectorType.SPARSE) {
...
async function createTable(connection, tableName, embeddingDim, customization) {
...
function _getIndexName(baseName) {
    const uniqueId = node_crypto_1.default.randomUUID().replace(/-/g, "");
...
function convertDenseVectorForFormat(values, format) {
    switch (format) {
        case exports.VectorElementFormat.FLOAT32:
        case undefined:
            return new Float32Array(values);
        case exports.VectorElementFormat.FLOAT64:
            return new Float64Array(values);
        case exports.VectorElementFormat.INT8: {
        ...
        case exports.VectorElementFormat.BINARY:
...
async function createIndex(client, vectorStore, params) {
...
async function dropTablePurge(connection, tableName) {
    try {
...
exports.dropTablePurge = dropTablePurge;
class OracleVS extends vectorstores_1.VectorStore {
    _vectorstoreType() {
        return "oraclevs";
    }
    constructor(embeddings, dbConfig) {
        ...
        Object.defineProperty(this, "distanceStrategy", {
            ...
            configurable: true,
            writable: true,
            value: exports.DistanceStrategy.COSINE
        });
    ...
    getActiveVectorType() {
        return this.vectorType ? normalizeVectorTypeValue(this.vectorType) : exports.VectorType.DENSE;
    }
    ...
    prepareVectorForStorage(vector) {
        ...
        if (vectorType === exports.VectorType.SPARSE) {
            ...
            if (format === exports.VectorElementFormat.INT8) {
            ...
            else if (format === exports.VectorElementFormat.FLOAT64) {
                sparseInput = new Float64Array(vector);
            }
            else if (format === exports.VectorElementFormat.FLOAT32) {
                sparseInput = new Float32Array(vector);
            }
            return new oracledb_1.default.SparseVector(sparseInput);
        }
        if (vector.length !== dimension) {
    ...
    async addVectors(vectors, documents, options) {
        ...
        try {
            ...
            for (let index = 0; index < documents.length; index += 1) {
                ...
                const externalId = inputIds?.[index] ?? doc.metadata.id ?? node_crypto_1.default.randomUUID();
      ...
      WHEN NOT MATCHED THEN
        ...
        VALUES (s.external_id, s.embedding, s.metadata, s.text)`;
            ...
            const executeOptions = {
                bindDefs: {
                    ext_id: { type: oracledb_1.default.STRING, maxSize: 255 },
                    text: { type: oracledb_1.default.STRING, maxSize: 10000000 },
    ...
    async maxMarginalRelevanceSearchWithScoreByVector(embedding, options) {
        ...
        const mmrSelectedIndices = (0, math_1.maximalMarginalRelevance)(queryEmbedding, consistentEmbeddings, lambdaMult, options.k);
