...
export class ChildProcessProxyWorker {
    ...
    async handleInit(message) {
        try {
            ...
            const pluginInjector = provideLogging(await provideLoggingClient(this.injector, message.loggingServerAddress, minPriority(message.options.logLevel, message.options.fileLogLevel)))
