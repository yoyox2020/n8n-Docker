...
function describeError(error) {
    ...
    switch (error.keyword) {
        ...
        case 'enum':
            return `${errorPrefix} should be one of the allowed values (${new Intl.ListFormat('en', { type: 'disjunction' }).format(error.params.allowedValues.map(stringify))}), but was ${stringify(error.data)}.`;
        case 'minimum':
        case 'maximum':
